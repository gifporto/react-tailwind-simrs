...
export default function CpptPage() {
    ...
    const handleDelete = async () => {
        ...
        setActionLoading(true)
        try {
            // Backend membutuhkan object { type: "SOAP" | "SBAR" | "ADIME" }
            // Pastikan selectedType sudah diatur saat tombol hapus diklik
            const payload = { type: selectedType }

            // Perhatikan: Pada Axios DELETE, payload harus dibungkus dalam properti 'data'
            // Jika library API Anda menggunakan axios, pastikan strukturnya seperti ini:
            await RanapAPI.deleteCppt(id, selectedIdForAction.toString(), { data: payload })
            ...
            const errorMsg = error.response?.data?.meta?.message || "Gagal menghapus catatan"
    ...
    const renderCpptContent = (item: CpptItem) => {
        ...
        const contentClass = cn("space-y-1", isDeleted && "line-through opacity-40 italic select-none")

        switch (item.type) {
            case "SOAP":
                return (
                    <div className={contentClass}>
                        <p className="text-sm"><span className="font-bold">S:</span> {d.subjective}</p>
                        <p className="text-sm"><span className="font-bold">O:</span> {d.objective}</p>
                        <p className="text-sm"><span className="font-bold">A:</span> {d.assessment}</p>
                        <p className="text-sm"><span className="font-bold">P:</span> {d.planning}</p>
                    </div>
                )
            case "SBAR":
                return (
                    <div className={contentClass}>
                        <p className="text-sm"><span className="font-bold">S:</span> {d.situation}</p>
                        <p className="text-sm"><span className="font-bold">B:</span> {d.background}</p>
                        <p className="text-sm"><span className="font-bold">A:</span> {d.assessment}</p>
                        <p className="text-sm"><span className="font-bold">R:</span> {d.recommendation}</p>
                    </div>
                )
            case "ADIME":
                return (
                    <div className={contentClass}>
                        <p className="text-sm"><span className="font-bold">A:</span> {d.assessment}</p>
                        <p className="text-sm"><span className="font-bold">D:</span> {d.diagnosis}</p>
                        <p className="text-sm"><span className="font-bold">I:</span> {d.intervention}</p>
                        <p className="text-sm"><span className="font-bold">ME:</span> {d.monitoring_evaluation}</p>
                    </div>
                )
        }
    }
    ...
    return (
        <div className="p-6 max-w-5xl mx-auto space-y-6 antialiased">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">CPPT</h1>
                    <p className="text-slate-500 text-sm">Catatan Perkembangan Pasien Terintegrasi (Terbaru di Atas)</p>
                </div>
                <Button onClick={() => { setIsEditing(null); setFormData({}); setIsModalOpen(true); }}>
                    <Plus className="w-4 h-4" /> Tambah CPPT
                </Button>
            </div>
                    ...
                    <div className="space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-slate-200">
                        {cpptData.map((item) => (
                            <div key={item.id} className="relative pl-12">
                                {/* Dot Marker */}
                                <div className={cn(
                                    "absolute left-0 mt-1.5 h-10 w-10 rounded-full border-4 border-white flex items-center justify-center shadow-md z-10 transition-all",
                                    item.deleted_at ? "bg-slate-300" : item.verified_at ? 'bg-emerald-500' : 'bg-slate-500'
                                )}>
                                ...
                                <Card className={cn(
                                    ...
                                    item.deleted_at ? "bg-slate-50 border-dashed border-slate-300 grayscale" :
                                    ...
                                    <CardHeader className="w-full flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <Badge variant={item.deleted_at ? "outline" : "default"}>
                                                {item.type} {item.deleted_at && "• DIHAPUS"}
                                            </Badge>
                                            {item.id}
                                        ...
                                        {!item.deleted_at && (
                                            <div className="flex items-center gap-1">
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => {
                                                        setSelectedIdForAction(item.id);
                                                        setSelectedType(item.type); // <--- PASTIKAN INI ADA
                                    ...
                                    <CardContent className="space-y-4">
                                        ...
                                        {item.notes && !item.deleted_at && (
                                            <div className="bg-amber-50 p-3 rounded-lg border border-amber-100 flex gap-2 animate-in fade-in slide-in-from-left-2">
                                        ...
                                        <div className="flex items-center justify-between text-[10px] uppercase font-bold tracking-widest text-slate-400">
                                            ...
                                            {item.verified_at && !item.deleted_at && (
                                                <div className="text-emerald-600 flex items-center gap-1">
                                                    <CheckCheck className="w-3.5 h-3.5" /> Verified at {item.verified_at}
                                                </div>
                                            )}
                                        </div>
                                    </CardContent>
            ...
            <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                <DialogContent className="sm:max-w-[500px]">
                    <DialogHeader><DialogTitle>{isEditing ? 'Perbarui' : 'Tambah'} CPPT</DialogTitle></DialogHeader>
                    ...
                    <DialogFooter className="mt-4 border-t pt-4">
                        ...
                        <Button onClick={handleSave} disabled={actionLoading}>
                            {actionLoading && <Loader2 className="w-4 h-4 animate-spin mr-2" />} Simpan CPPT
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>

            {/* Dialog Verifikasi Note */}
