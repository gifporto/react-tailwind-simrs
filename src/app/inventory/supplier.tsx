...
import {
  ...
  useUpdateSupplier,
  useDeleteSupplier
} from "@/hooks/queries/use-supplier-queries";
...
import { Search, Store, Loader2, Plus, Edit, Trash2, Phone, MapPin, Mail, User } from "lucide-react";
...
export default function SupplierIndexPage() {
  ...
  /* =======================
     FETCH DATA
  ======================= */
  const { data: apiResponse, isLoading, isError, refetch } = useSupplierList(page, perPage, search);
  ...
  const handleOpenDialog = (supplier?: any) => {
    if (supplier) {
      ...
      setFormData({ nama_supplier: "", telp: "", alamat: "", email: "", contact_person: "" });
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const options = {
      onSuccess: () => {
        toast.success(selectedSupplier ? "Supplier diperbarui" : "Supplier ditambahkan");
    ...
    if (selectedSupplier) {
      updateMutation.mutate({ id: selectedSupplier.id, payload: formData }, options);
    } else {
    ...
    >
            ...
            <div className="space-y-4 w-full">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Kelola daftar pemasok barang/obat dan informasi kontak.
                </p>
              </div>
          ...
          <AnimatePresence mode="wait">
            {isLoading && listData.length === 0 ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="mt-4 text-muted-foreground font-medium">Data supplier tidak ditemukan</p>
                {isError && (
                    ...
                    <TableBody>
                      {listData.map((item: any, i: number) => (
                        <TableRow key={item.id} className="hover:bg-muted/30 transition-colors">
                          ...
                          <TableCell>
                            <div className="font-semibold text-sm">{item.nama_supplier}</div>
                            {item.is_aktif !== undefined && (
                              <Badge variant={item.is_aktif ? "default" : "secondary"} className="mt-1 text-xs">
                          ...
                          <TableCell>
                            <div className="flex flex-col gap-1">
                              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                <Phone className="w-3 h-3" /> {item.telp || "-"}
                              </div>
                              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                              ...
                              {item.contact_person && (
                                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                  <User className="w-3 h-3" /> {item.contact_person}
                                </div>
                              )}
                            </div>
                          </TableCell>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>{selectedSupplier ? "Edit Supplier" : "Tambah Supplier Baru"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4 pt-4">
            <div className="space-y-2">
              <Label htmlFor="nama_supplier">Nama Supplier / Perusahaan</Label>
              <Input
                id="nama_supplier"
                required
                value={formData.nama_supplier}
                onChange={(e) => setFormData({ ...formData, nama_supplier: e.target.value })}
                placeholder="Contoh: PT. Indofarma Tbk"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Contoh: 083772847428 (Andri)"
              />
            </div>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tindakan ini akan menghapus <strong>{selectedSupplier?.nama_supplier}</strong> dari sistem secara permanen.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
